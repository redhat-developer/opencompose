...
__opencompose_type() {
	...
	if [ "$1" == "-t" ]; then
		...
		# all the time
		if [ "$1" = "__opencompose_compopt" ]; then
			echo builtin
			return 0
		fi
	fi
...
__opencompose_compopt() {
	true # don't do anything. Not supported by bashcompinit in zsh
}
__opencompose_declare() {
	if [ "$1" == "-F" ]; then
		whence -w "$@"
	else
		builtin declare "$@"
...
__opencompose_ltrim_colon_completions()
{
...
__opencompose_get_comp_words_by_ref() {
	cur="${COMP_WORDS[COMP_CWORD]}"
	prev="${COMP_WORDS[${COMP_CWORD}-1]}"
	words=("${COMP_WORDS[@]}")
	cword=("${COMP_CWORD[@]}")
}
__opencompose_filedir() {
...
__opencompose_convert_bash_to_zsh() {
	sed \
	-e 's/declare -F/whence -w/' \
	-e 's/local \([a-zA-Z0-9_]*\)=/local \1; \1=/' \
	-e 's/flags+=("\(--.*\)=")/flags+=("\1"); two_word_flags+=("\1")/' \
	-e 's/must_have_one_flag+=("\(--.*\)=")/must_have_one_flag+=("\1")/' \
	-e "s/${LWORD}_filedir${RWORD}/__opencompose_filedir/g" \
	-e "s/${LWORD}_get_comp_words_by_ref${RWORD}/__opencompose_get_comp_words_by_ref/g" \
	-e "s/${LWORD}__ltrim_colon_completions${RWORD}/__opencompose_ltrim_colon_completions/g" \
	-e "s/${LWORD}compgen${RWORD}/__opencompose_compgen/g" \
	-e "s/${LWORD}compopt${RWORD}/__opencompose_compopt/g" \
	-e "s/${LWORD}declare${RWORD}/__opencompose_declare/g" \
	-e "s/\\\$(type${RWORD}/\$(__opencompose_type/g" \
	<<'BASH_COMPLETION_EOF'
`
...
__opencompose_bash_source <(__opencompose_convert_bash_to_zsh)
`
